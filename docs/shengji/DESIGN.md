...
- 最后一圈（手牌出完的那一圈）由闲家赢：按**首出最大牌型**计算底牌番数，纯散牌甩仍为单抠 ×2，含对子为双抠 ×4，
  含拖拉机按最长拖拉机张数计算 `2^n`（两连对四张 ×16），上限 ×64。盖毙不会把首出的单抠变成双抠。计入闲家得分。
- 闲家得分 s（含抠底与罚分）→ 结果：
...
- **跟牌**：单张、对子、同长度拖拉机都枚举“最小能赢”；对家只有在自己是末家时才算真正赢定并垫分，后面还有对手时不冒险提前送分。
  缺门时有分且能毙就用最小能毙的主，否则保主、保分、垫小牌。
...
- 选牌：**单击一张即按“最长连对 > 对子 > 单张”预选完整单位**；跟牌时优先补成包含该牌的一手合法出法。
